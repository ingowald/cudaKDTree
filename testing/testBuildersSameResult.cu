...
template<typename T, int D>
struct PointWithPayloadAndDim_traits : public cukd::default_data_traits<vecN<T,D>> {

  /*! @{ just for our own readability: */
  using inherited    = cukd::default_data_traits<vecN<T,D>>;
  using data_t       = PointWithPayloadAndDim<T,D>;
  using point_traits = typename inherited::point_traits;
  using point_t      = typename inherited::point_t;
  /*! @] */
...
template<typename T, int D>
void test_withPayloadAndDim(int numPoints)
{
  std::cout << "testing for " << numPoints
            << " inputs, type = " << typeToString<T>() << D << "+payload"
            << std::endl;
  ...
  std::vector<data_t> data_host = inputData;
  cukd::buildTree_host
    <PointWithPayloadAndDim<T,D>,PointWithPayloadAndDim_traits<T,D>>
    (data_host.data(),numPoints,d_bounds);
...
void testN(int sizeToTest)
{
  ...
  test_withPayloadAndDim<float,N>(sizeToTest);
  test_withPayloadAndDim<int,N>(sizeToTest);
}
